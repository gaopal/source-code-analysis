...
public class Collections {
  ...
  /**
   * max 方法泛型 T 定义得非常巧妙
   * 泛型必须继承 Object 并且实现 Comparable 的接口。
   * 这里泛型直接定义了必须实现 Comparable 接口,在编译的时候就可告诉使用者,当前类没有实现 Comparable 接口
   *
   ...
   * interface. Furthermore, all elements in the collection must be <i>mutually comparable</i> (that
   * is, <tt>e1.compareTo(e2)</tt> must not throw a <tt>ClassCastException</tt> for any elements
   * <tt>e1</tt> and <tt>e2</tt> in the collection).
   *
   * <p>This method iterates over the entire collection, hence it requires time proportional to the
